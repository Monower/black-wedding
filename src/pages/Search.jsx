            ...
            <div>
              <div
                className={`flex flex-wrap gap-6 pb-10 ${
                  viewType == "map" ? "justify-around" : "justify-center"
                }`}
              >
                {data?.slice(fromItem, toItem)?.map((item, index) => {
                  if (viewType == "grid") {
                    ...
                    return (
                      <div
                        key={index}
                        className="w-[45%] lg:w-[40%] bg-white hover:drop-shadow-lg  rounded-2xl transition-all transform duration-500 pb-3 group overflow-hidden"
                      >
                        <VendorCard
                          image={item.image}
                          topRated={item.topRated}
